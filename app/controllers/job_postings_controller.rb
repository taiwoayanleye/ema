...
# GET /job_postings.json
  ...
  if params[:position_text]
    if params[:postion_text] != "" and params[:description_text] != "" and params[:paid_text]
      and params[:requirements_text] != ""
      match_term1 = "%" + params[:position_text] + "%"
      match_term2 = "%" + params[:postion_text] + "%"
      match_term3 = params[:paid_text]
      match_term4 = "%" + params[:requirements_text] + "%"
      @job_postings = JobPosting.where("Position LIKE ? AND Job_decription LIKE ? AND Job_paid = ? AND Job_requirements LIKE ?", match_term1, match_term2, match_term3, match_term4)
    elsif params[:postion_text] != "" and params[:description_text] != "" and params[:paid_text] != ""
      match_term1 = "%" + params[:position_text] + "%"
      match_term2 = "%" + params[:description_text] + "%"
      match_term3 = "%" +params[:paid_text]
      @job_postings = JobPosting.where("Position LIKE ? AND Job_decription LIKE ? AND Job_paid = ?", match_term1, match_term2, match_term3)
    elsif params[:postition_text] != "" and params[:description_text] != "" and params[:requirements_text] != ""
      match_term1 = "%" + params[:position_text] + "%"
      match_term2 = "%" +params[:decription_text] + "%"
      match_term3 = params[:paid_text]
      @job_postings = JobPosting.where("Postion LIKE ? AND Job_decription LIKE ? AND Job_paid = ?", match_term1, match_term2, match_term3)
    elsif params[:postion_text] != "" and params[:description_text] != "" and params[:requirements_text] != ""
      match_term1 = "%" + params[:position_text] + "%"
      match_term2 = "%" + params[:decription_text] + "%"
      match_term4 = "%" + params[:requirements_text] + "%"
      @job_postings = JobPosting.where("Postition LIKE ? AND Job_decription LIKE ? AND Job_requirements LIKE ?", match_term1, match_term2, match_term4)
    elsif params[:position_text] != "" and params[:paid_text] != "" and params[:requirements_text] != ""
      match_term1 = "%" + params[:postion_text] + "%"
      match_term3 = "%" + params[:paid_text]
      match_term4 = "%" + params[:requiremnets_text] + "%"
      @job_postings = JobPosting.where("Postiton LIKE ? AND Job_paid = ? AND Job_requirements LIKE ?", match_term1, match_term3, match_term4)
    elsif params[:description_text] != "" and params[:paid_text] != "" and params[:requiremnets_text] != ""
      match_term2 = "%" + params[:description_text] + "%"
      match_term3 = "%" + params[:paid_text]
      match_term4 = "%" + params[:requiremnets_text] + "%"
    ...
    elsif params[:postion_text] != "" and params[:decription_text] != ""
      match_term1 = "%" + params[:postion_text] + "%"
      match_term2 = "%" + params[:description_text] + "%"
      @job_postings = JobPostings.where("Position LIKE ? AND Job_description LIKE ?", match_term1, match_term2)
    elsif params[:position_text] != "" and params[:paid_text] !=
      match_term1 = "%" + params[:postition_text] + "%"
      match_term3 = "%" + params[:paid_text]
      @job_postings = JobPosting.where("Position LIKE ? AND Job_paid = ?", match_term1, match_term3)
    elsif params[:postion_text] != "" and params[:requirements_text] != ""
      match_term1 = "%" + params[:postion_text] + "%"
      match_term4 = "%" + params[:requirements_text] + "%"
      ...
      match_term2 = "%" + params[:decription_text] + "%"
      match_term3 = "%" + params[:paid_text]
      @job_postings = JobPosting.where("Job_decription LIKE ? AND Job_paid LIKE ?", match_term2, match_term3)
      ...
      match_term3 = params[:paid_text]
      match_term4 = "%" + params[:requiremnets_text] + "%"
      @job_postings = JobPosting.where("Job_paid = ? AND Job_requiremnts LIKE ?", match_term3, match_term4)
    elsif params[:position_text] != ""
      match_term1 = "%" + params[:position_text] + "%"
      @job_postings = JobPosting.where("Position LIKE ?", match_term1)
    elsif params[:decription_text] != ""
      match_term2 = "%" + params[:decription_text] + "%"
      @job_postings = JobPosting.where("Job_description LIKE ?", match_term2)
    elsif params[:paid_text] != ""
      match_term3 = params[:paid_text]
      @job_postings = JobPosting.where("Job_paid = ?", mathc_term3)
    elsif params[:requiremnets_text] != ""
      match_term4 = "%" + params[:requiremnets_text] + "%"
      @job_postings = JobPostings.where("Job_requirements LIKE ?", match_term4)
    end
  end
end
